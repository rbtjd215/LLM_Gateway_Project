...
KOREAN_FILES = [
    "phase1_Korean.csv", # Phase 1: NORMAL
    "phase2_Korean.csv", # Phase 2: LEAK_PLAIN
    "phase3_Korean.csv", # Phase 3: LEAK_EVASION
    "phase4_Korean.csv", # Phase 4: INJECTION
]
...
async def run_korean_bulk_test():
    print("=" * 60)
    print("🚀 [대규모 검증] 한국어 프롬프트 600개 V4 파이프라인 테스트 시작")
    print("=" * 60)
    ...
    print(f"\n총 {len(all_prompts)}개의 한국어 프롬프트가 병합되었습니다.")
    print("Ollama(Qwen2.5)에 대한 순차 처리를 시작합니다. (약 15~20분 소요 예상)")
    print("-" * 60)
    
    start_time = time.time()
    
    results = []
    # 안정성을 위해 순차 처리 (Semaphore=1)
    ...
    async with security_lifespan(None):
        for idx, (p_num, expected, text) in enumerate(all_prompts):
            ...
            df_temp.to_csv("korean_v4_test_results.csv", mode=write_mode, header=write_header, index=False, encoding='utf-8-sig')
            
    end_time = time.time()
    print(f"\n✅ 600개 프롬프트 검증 완료! (총 소요 시간: {end_time - start_time:.2f}초)")
    print("결과가 'korean_v4_test_results.csv'에 최종 저장되었습니다.\n")
    
    df_res = pd.DataFrame(results)
    
    print("=" * 60)
    print("📊 [한국어 600개 통합 테스트 결과 (Phase 1~4)]")
    print("=" * 60)
...
if __name__ == "__main__":
    asyncio.run(run_korean_bulk_test())
