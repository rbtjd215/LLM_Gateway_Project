...
> **병렬 처리 최적화 및 인프라 확장성 (Scalability)**
> 무거운 LLM 연산인 Phase 2와 Phase 3은 `asyncio.gather()`를 통해 **완전 병렬로 동시 실행**된다. 이러한 설계 덕분에, 향후 인프라(GPU 등)가 확장되었을 때 파이프라인의 병목 없이 처리 속도가 자원 투입량에 비례하여 비약적으로 상승하는 뛰어난 확장성을 보여준다.
